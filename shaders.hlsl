...
[[vk::push_constant]]
#endif
CONSTANT_BUFFER(constants, per_mesh, b0);

pixel
vs(uint index_id : SV_VertexID)
{
    uint vertex_id = indices[per_mesh.index_offset + index_id];
    vertex v = vertices[per_mesh.vertex_offset + vertex_id];
...
{
    material_properties mp = properties[per_mesh.material_id];

    // Sample textures.
#if defined(BINDLESS)
    uint tex_offset = per_mesh.texture_offset;
#else
    uint tex_offset = 0;
#endif
