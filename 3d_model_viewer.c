...
typedef struct
{
    ...
    u32 max_mesh_count;
    u32 max_model_vertex_count;
    u32 max_model_index_count;
    pg_f32_3x* scaling;
} models_metadata;
...
GLOBAL pg_config config = {.gamepad_count = 1,
                           ...
                           .transient_mem_size = 50u * PG_MEBIBYTE,
...
GLOBAL application_state app_state
    = {.vsync = true,
       ...
       .gfx_api = PG_GRAPHICS_API_VULKAN};
...
reset_view(void)
{
    ...
    switch (app_state.model_id)
    {
        case MODEL_BAKER_AND_THE_BRIDGE:
        {
            ...
            app_state.camera.position.z = 5.0f;
            break;
        }
        case MODEL_CORSET:
        {
            app_state.camera.position.y = PG_PI / 3.0f;
            app_state.camera.position.z = 9.0f;
            break;
        }
        ...
        case MODEL_FTM:
        {
            app_state.rotation = (pg_f32_3x){.x = 0.0f, .y = 135.0f, .z = 0.0f};
            app_state.camera.position.y = PG_PI / 2.5f;
            app_state.camera.position.z = 2.25f;
            break;
        }
        case MODEL_METAL_ROUGH_SPHERES:
        {
            app_state.camera.position.z = 7.0f;
            break;
        }
...
init_app(pg_assets* assets,
         ...
         pg_scratch_allocator* model_mem,
         ...
         pg_error* err)
{
    ...
    {
        if (assets->models[i].mesh_count > metadata->max_mesh_count)
        {
            metadata->max_mesh_count = assets->models[i].mesh_count;
        }

        if (assets->models[i].material_count > metadata->max_material_count)
        {
            metadata->max_material_count = assets->models[i].material_count;
        }

        u32 model_vertex_count = 0;
        u32 model_index_count = 0;
        f32 norm_scale = 0.0f;
        for (u32 j = 0; j < assets->models[i].mesh_count; j += 1)
        {
            pg_asset_mesh* mesh = &assets->models[i].meshes[j];

            model_vertex_count += mesh->vertex_count;
            model_index_count += mesh->index_count;

            for (u32 k = 0; k < mesh->vertex_count; k += 1)
            {
                pg_f32_3x position = mesh->vertices[j].position;
        ...
        if (model_vertex_count > metadata->max_model_vertex_count)
        {
            metadata->max_model_vertex_count = model_vertex_count;
        }
        if (model_index_count > metadata->max_model_index_count)
        {
            metadata->max_model_index_count = model_index_count;
        }
        ...
        pg_graphics_message gfx_msgs[] = {
            ...
            {.type = PG_GRAPHICS_MESSAGE_TYPE_BUFFER,
             .buffer_msg = {.structured = true,
                            ...
                            .shader_register = 0,
                            .shader_stage = PG_SHADER_STAGE_VERTEX,
                            .elem_count = metadata->max_model_vertex_count,
            ...
            {.type = PG_GRAPHICS_MESSAGE_TYPE_BUFFER,
             .buffer_msg = {.structured = true,
                            ...
                            .shader_register = 1,
                            .shader_stage = PG_SHADER_STAGE_VERTEX,
                            .elem_count = metadata->max_model_index_count,
    ...
    ok &= pg_scratch_init(permanent_mem, 500, model_mem);
    if (!ok)
    {
        err->log(err,
                 PG_ERROR_MAJOR,
                 "init_app: failed to initialize model memory");
    }
    ...
    // Create update graphics messages.
    {
        ...
        if (metadata->model_id_last_frame != app_state.model_id)
        {
            ...
            // Update vertex/index structured buffers.
            {
                u32 model_vertex_count = 0;
                u32 model_index_count = 0;
                ok &= pg_scratch_alloc(model_mem,
                                       model->mesh_count * sizeof(u32),
                                       alignof(u32),
                                       vertex_offsets);
                ok &= pg_scratch_alloc(model_mem,
                                       model->mesh_count * sizeof(u32),
                ...
                if (!ok)
                {
                    err->log(
                        err,
                        PG_ERROR_MAJOR,
                        "update_app: failed to get memory for vertex/index "
                        "offsets");
                }
                for (u32 i = 0; i < model->mesh_count; i += 1)
                {
                    (*vertex_offsets)[i] = model_vertex_count;
                    model_vertex_count += model->meshes[i].vertex_count;

                    (*index_offsets)[i] = model_index_count;
                    model_index_count += model->meshes[i].index_count;
                }

                pg_vertex* vertices;
                PG_GRAPHICS_INDEX_TYPE* indices;
                ok &= pg_scratch_alloc(transient_mem,
                                       model_vertex_count * sizeof(pg_vertex),
                                       alignof(pg_vertex),
                                       &vertices);
                ok &= pg_scratch_alloc(transient_mem,
                                       model_index_count
                                           * sizeof(PG_GRAPHICS_INDEX_TYPE),
                                       alignof(PG_GRAPHICS_INDEX_TYPE),
                                       &indices);
                if (!ok)
                {
                    err->log(err,
                             PG_ERROR_MAJOR,
                             "update_app: failed to get memory for "
                             "vertices/indices");
                }

                for (u32 i = 0; i < model->mesh_count; i += 1)
                {
                    u32 vertex_offset = (*vertex_offsets)[i];
                    ok &= pg_copy(model->meshes[i].vertices,
                                  model->meshes[i].vertex_count
                                      * sizeof(pg_vertex),
                                  vertices + vertex_offset,
                                  (model_vertex_count - vertex_offset)
                                      * sizeof(pg_vertex));

                    u32 index_offset = (*index_offsets)[i];
                    ok &= pg_copy(model->meshes[i].indices,
                                  model->meshes[i].index_count
                                      * sizeof(PG_GRAPHICS_INDEX_TYPE),
                                  indices + index_offset,
                                  (model_index_count - index_offset)
                                      * sizeof(PG_GRAPHICS_INDEX_TYPE));
                }
                if (!ok)
                {
                    err->log(err,
                             PG_ERROR_MAJOR,
                             "update_app: failed to copy vertex/index data");
                }
                ...
                gfx_msgs[gfx_msg_count].buffer_msg.elem_count
                    = model_vertex_count;
                gfx_msgs[gfx_msg_count].buffer_msg.data = vertices;
                gfx_msg_count += 1;
                ...
                gfx_msgs[gfx_msg_count].buffer_msg.elem_count
                    = model_index_count;
                gfx_msgs[gfx_msg_count].buffer_msg.data = indices;
                gfx_msg_count += 1;
            }
...
wWinMain(HINSTANCE inst, HINSTANCE prev_inst, WCHAR* cmd_args, s32 show_code)
{
    ...
    pg_scratch_allocator model_mem = {0};
    u32* vertex_offsets = 0;
    u32* index_offsets = 0;
